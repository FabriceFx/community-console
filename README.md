...
- **Traitement des relances** : Bouton **« 💬 Répondre à la relance »** lorsque la personne a répondu après vous. Le texte est inséré via **« Ajouter un commentaire »** placé sous le message du demandeur — et non via « Répondre », qui ouvrirait une seconde réponse au fil. Aucun clic préalable sur *Suivre dans Sheets* n'est nécessaire : un fil inconnu est inscrit automatiquement, sans générer de réponse initiale inutile. La relance est classée (`RESOLU`, `ECHEC`, `INCOMPRIS`, `NOUVEAU`, `HORS_SUJET`) et la réponse est adaptée à chaque cas — un remerciement appelle deux phrases, un échec interdit de reproposer la même manipulation. Un garde-fou mesure le recouvrement avec votre réponse précédente et vous alerte si la proposition la reformule. La colonne *Date de relance* et le statut *Résolue* sont mis à jour automatiquement.
- **Intervention sur un fil déjà répondu par un autre bénévole** : Bouton **« 💬 Compléter le fil »**. La consigne exige une justification — point factuel erroné, élément manquant, étape à reformuler — et autorise explicitement le verdict **« rien à ajouter »**, auquel cas aucun texte n'est inséré. Un message redondant encombre le fil et dessert la personne.
